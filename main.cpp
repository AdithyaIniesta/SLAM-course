...
// set sensor marcos to "0" to disable them
...
int main() {
    ...
    std::ifstream sensor_file(
            "/home/adithyainiesta/Documents/EKF Vehicle Tracking/sensor_measurements/obj_pose-laser-radar-synthetic-input.txt");
    if (!sensor_file.is_open()) {
        ...
        for (int i = 0; i < N; i++) {
            sensor_tracking.process_measurements(
                    &sensor_tracking.package_list[i]);
        }
    }

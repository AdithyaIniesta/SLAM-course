...
tracking::tracking() {
    ...
    outputFile.open(
            "/home/adithyainiesta/Documents/EKF Vehicle Tracking/output/output.csv");
    outputFile << "x, y, sigmaxx, sigmaxy, sigmayx, sigmayy,\n";
...
void tracking::process_measurements(measurementPackage *mp) {
    ...
    if (first_measurement) {
        ...
        Eigen::MatrixXd P = kf.printCov();
        outputFile << m(0) << "," << m(1) << "," << P(0, 0) << ","
                   << P(0, 1) << "," << P(1, 0) << ","
                   << P(1, 1) << "," << "\n";
    }
}
